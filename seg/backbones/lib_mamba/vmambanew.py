...
class SS2Dv2:
    ...
    def forward_corev2(
            self,
            x: torch.Tensor = None,  # 输入张量，形状 (B, D, H, W)
            force_fp32=False,  # 是否强制将输入转为 float32
            ssoflex=True,  # selective_scan 输出 dtype 设置，True→输出32位
            no_einsum=False,  # 是否用 conv1d/linear 代替 einsum
            selective_scan_backend=None,  # 后端选项："mamba"/"oflex"/"core"/"torch"/None
            scan_mode="cross2d",  # 扫描模式："cross2d"/"unidi"/"bidi"/"cascade2d"
            scan_force_torch=False,  # 是否强制使用纯 PyTorch 实现
            **kwargs,
    ):
        # 检查参数合法性
        assert scan_mode in ["unidi", "bidi", "cross2d", "cascade2d"]
        assert selective_scan_backend in [None, "oflex", "core", "mamba", "torch"]
        delta_softplus = True  # selective_scan 内部 softplus 标志
        out_norm = self.out_norm  # 输出归一化/激活模块
        channel_first = self.channel_first  # 布局标志
        # 工具函数：一组张量转 float32
        to_fp32 = lambda *args: (_a.to(torch.float32) for _a in args)

        # 解包尺寸
        B, D, H, W = x.shape  # Batch, Channels, Height, Width
        D, N = self.A_logs.shape  # A_logs 里 (K*d_inner, d_state)
        K, D, R = self.dt_projs_weight.shape  # K组, d_inner, dt_rank
        L = H * W  # 空间扁平长度

        # 将模式名转为数值 index
        _scan_mode = dict(cross2d=0, unidi=1, bidi=2, cascade2d=3)[scan_mode]

        # selective_scan 的局部包装
        def selective_scan(u, delta, A, B, C, D=None, delta_bias=None, delta_softplus=True):
            return selective_scan_fn(
                u, delta, A, B, C, D, delta_bias, delta_softplus,
                ssoflex, backend=selective_scan_backend
            )

        # 若为 cascade2d 模式，拆分为先行后列两次扫描
        if _scan_mode == 3:
            x_proj_bias = getattr(self, "x_proj_bias", None)  # 可选偏置
            ...
            y_row = F.layer_norm(
                ...
                normalized_shape=(int(y_row.shape[1]),)
            ).permute(0, 3, 1, 2).contiguous()
            ...
            y_col = scan_rowcol(
                ...
                proj_bias=(x_proj_bias.view(self.k_group, -1)[2:].contiguous().to(
            ...
            y = y_col  # 最终输出 (B, C, H, W)

        else:
            # 其它模式：cross2d / unidi / bidi
            x_proj_bias = getattr(self, "x_proj_bias", None)

            # 先做 cross_scan（Triton kernel）
            if self.k_group == 4:
                xs = cross_scan_fn(
                    x, in_channel_first=True, out_channel_first=True,
        ...
        if not channel_first:
            y = y.view(B, -1, H * W).contiguous() \
                ...
                .view(B, H, W, -1).contiguous()
        # 应用输出归一化/激活
        y = out_norm(y)

        return y.to(x.dtype)  # 恢复与输入相同的 dtype
