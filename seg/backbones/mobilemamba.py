...
def extract_sliding_patches(x, patch_size=64, stride=32):
    ...
    for i in range(n_h):
        for j in range(n_w):
            h_start = i * stride
            w_start = j * stride
            patches_positions.append((h_start, h_start + patch_size,
...
def reconstruct_from_patches(patches, patches_positions, original_size, stride=32):
    ...
    for i, (h0, h1, w0, w1) in enumerate(patches_positions):
        output[:, :, h0:h1, w0:w1] += patches[:, i]
        count[:, :, h0:h1, w0:w1] += 1
...
class PatchedSS2D(nn.Module):
    ...
    def forward(self, x):
        ...
        B0, N, C, P_H, P_W = patches.shape
        flat = patches.view(B0 * N, C, P_H, P_W)
        proc = self.original_ss2d(flat).view(B0, N, C, P_H, P_W)
        out = reconstruct_from_patches(proc, patches_positions, (H, W), self.stride)
        return out
...
def patch_mbwtconv2d_model(model, patch_size=64, stride=32):
    model = patch_ss2d_modules(model, patch_size, stride)
    for module in model.modules():
        if hasattr(module, 'global_atten') and isinstance(module.global_atten, PatchedSS2D):
            module.forward = MethodType(modify_mbwtconv2d_forward, module)
    return model
...
class MBWTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True,
        ...
        self.wt_levels = wt_levels  # 小波变换层级数
        self.stride = stride  # 下采样步长
        self.dilation = 1  # 空洞率（未使用）
        # 保存 patch 参数
        self.patch_size = patch_size
        self.patch_stride = patch_stride
        # 创建固定的小波滤波器（不可训练参数）
        self.wt_filter, self.iwt_filter = create_wavelet_filter(wt_type, in_channels,
                                                                in_channels, torch.float)
        #可选择是否训练滤波器参数
        self.wt_filter = nn.Parameter(self.wt_filter, requires_grad=False)  # 小波正变换滤波器
        self.iwt_filter = nn.Parameter(self.iwt_filter, requires_grad=False)  # 小波逆变换滤波器
        # 包装小波变换函数（参数固化：partial）
        self.wt_function = partial(wavelet_transform, filters=self.wt_filter)  # 小波分解
        self.iwt_function = partial(inverse_wavelet_transform, filters=self.iwt_filter)  # 小波重构
        # 全局注意力模块（基于状态空间模型SS2D）
        self.global_atten = SS2D(
            d_model=in_channels,
            d_state=1,  # 状态维度
            ssm_ratio=ssm_ratio,  # 状态压缩率
            initialize="v2",  # 参数初始化方式
            forward_type=forward_type,  # 前向传播类型
            channel_first=True,  # 通道维度在前
            k_group=2  # 分组卷积数
        )
        self.global_atten = PatchedSS2D(
            self.global_atten,
        ...
        self.wavelet_convs = nn.ModuleList(
            [nn.Conv2d(
                in_channels * 4,  # 输入通道（LL/LH/HL/HH四通道）
                in_channels * 4,  # 输出通道
                kernel_size,
                padding='same',  # 保持空间尺寸
                stride=1,
                dilation=1,
                groups=in_channels * 4,  # 深度可分离卷积（每组1通道）
                bias=False)
                for _ in range(self.wt_levels)]
        )
        # 构建多级缩放系数（每层独立学习） 多级通道注意力
        self.wavelet_scale = nn.ModuleList(
            [_ScaleModule([1, in_channels * 4, 1, 1], init_scale=0.1)  # 初始化缩放系数为0.1
             for _ in range(self.wt_levels)]
        )
        # 下采样处理（当stride>1时启用）
        if self.stride > 1:
    ...
    def forward(self, x):
        ...
        # 最终重构结果
        x_tag = next_x_ll
        assert len(x_ll_in_levels) == 0  # 确保所有层级处理完毕
